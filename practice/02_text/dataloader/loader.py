    ...
    def load_imdb_data(directory = 'train', datafile = None):
        ...
        for sentiment in ('pos', 'neg'):
            ...
            for review_file in os.listdir(path):
                ...
                tmp = pd.DataFrame([[utils.strip_html_tags(review), labels[sentiment]]]) # 수정완료
                preprocess_tmp.append(tmp) # 수정완료

        df = pd.concat(preprocess_tmp).reset_index(drop=True) # 수정완료
